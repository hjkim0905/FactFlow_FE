...
import type { SupportedAIModel, NewsAnalysisResponse, AnalysisResult } from '@/types/newAnalysis';
...
export class NewsAnalysisService {
    ...
    private async runAllAnalyses(content: string, title = ''): Promise<AnalysisResult> {
        ...
        try {
            ...
            results.title_rewrite = await this.executeWithRetry(() => this.chains.title.invoke(input));

            console.log('2/7 - 요약 생성');
            results.summary = await this.executeWithRetry(() => this.chains.summary.invoke(input));

            console.log('3/7 - 키워드 추출');
            results.keywords = await this.executeWithRetry(() => this.chains.keywords.invoke(input));

            console.log('4/7 - 난이도 분석');
            results.difficulty = await this.executeWithRetry(() => this.chains.difficulty.invoke(input));

            console.log('5/7 - 표현 분석');
            results.expression = await this.executeWithRetry(() => this.chains.expression.invoke(input));

            console.log('6/7 - 사고 질문 생성');
            results.thinking_questions = await this.executeWithRetry(() => this.chains.questions.invoke(input));
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt === maxRetries) {
                    console.warn(`최종 실패: ${error instanceof Error ? error.message : String(error)}`);
                    return {} as T; // 기본값 반환
                }
                console.warn(
