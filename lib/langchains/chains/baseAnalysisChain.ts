...
export abstract class BaseAnalysisChain {
	...
	protected parseJsonSafely(jsonString: string, fallback: unknown = {}): unknown {
		try {
			// 1. 기본 정리
	...
	private fixNestedQuotes(jsonStr: string): string {
		try {
			...
			// 더 포괄적인 필드 패턴 매칭
			const fieldPattern = /("(?:title|why_useful|summary|source|publisher|insight|icon|change_reason|original_title|category|authors|rewritten_title|text|keyword|expression|question|analysis|description|content)"\s*:\s*")([^"]*(?:\\"[^"]*)*?)("(?:\s*[,}]\s*))/g;

			fixed = fixed.replace(fieldPattern, (match, prefix, content, suffix) => {
				// 내용에서 이스케이프되지 않은 따옴표를 찾아 이스케이프
