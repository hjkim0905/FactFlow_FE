...
{keywords}
	...
	private isValidComplementaryInsight(result: unknown): boolean {
		...
		// 필수 필드 검증
		if (!insight.hasOwnProperty('complementary_articles') ||
			!insight.hasOwnProperty('insight')) {
		...
		if (!Array.isArray(insight.complementary_articles)) {
			console.warn("⚠️ [ComplementaryInsight] complementary_articles가 배열이 아님:", typeof insight.complementary_articles);
			return false;
		}
