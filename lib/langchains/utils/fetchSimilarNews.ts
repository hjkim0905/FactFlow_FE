...
import type { NewsItem, ProcessedNews } from "@/types/newAnalysis";
function cleanHTML(input: string): string {
    return input.replace(/<[^>]+>/g, "");
}
export async function fetchSimilarNews(
    summary: string,
    keywords: string[],
    count = 50
    ...
    try {
        ...
        console.error("❌ 네이버 뉴스 fetch 실패:", error.message);
        throw new Error(`뉴스 검색 중 오류 발생: ${error.message}`);
    }
}