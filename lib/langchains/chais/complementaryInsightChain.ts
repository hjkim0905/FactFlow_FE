...
{keywords}
    ...
    private sanitizeJsonString(str: string): string {
        try {
            ...
            // 2. 개행 문자 제거 (문자열 내부의 개행은 유지)
            cleaned = cleaned.replace(/\\(\r?\n|\n)/g, '');

            // 3. JSON 파싱 전 문자열 내 쌍따옴표 처리
            // 이미 이스케이프된 쌍따옴표는 그대로 두고, 잘못된 쌍따옴표만 수정
            ...
            console.warn('⚠️ sanitizeJsonString 실패, 원본 반환:', e);
            return str;
        }
    }
    ...
    private fixJsonQuotes(jsonStr: string): string {
        try {
            // 정규식을 사용하여 JSON 문자열 내의 쌍따옴표를 찾아 수정
            // "title": "내용에 "쌍따옴표" 있음" 형태를 처리
            let fixed = jsonStr;

            // 1. 문자열 값 내의 쌍따옴표를 임시로 치환
            const stringValueRegex = /("(?:title|why_useful|summary|source|publisher|insight|icon)"\s*:\s*")([^"]*(?:\\"[^"]*)*)(")/g;

            fixed = fixed.replace(stringValueRegex, (match, prefix, content, suffix) => {
                // 문자열 내용에서 이스케이프되지 않은 쌍따옴표를 이스케이프
    ...
    protected parseResult(result: string): unknown {
        ...
        const sanitized = this.sanitizeJsonString(decodedResult);

        // 2. 디버깅 로그
        console.log("✅ 디코딩된 LLM 결과:", sanitized);

        // 3. JSON 파싱

        return this.parseJsonSafely(sanitized, {
            complementary_articles: [],
            insight: '보완적 분석 없음',
        });
    }
