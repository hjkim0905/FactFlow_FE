	...
	return (
		<div className="bg-white shadow rounded-lg p-6 text-black relative overflow-hidden">
			...
			<Image
				...
				height={192}
			/>
			<div className="text-[14px] font-bold mb-4 flex items-center gap-2 z-10 relative">
			...
			<div className="mx-auto w-[270px] gap grid grid-cols-2 z-10 relative">
				{safeInfos.slice(0, 4).map((info, idx) => (
					<a
						...
						className={` flex flex-col items-center justify-center rounded-[20px] border-1 border-[#0073FF] bg-[#EFEFEF] p-4 w-[128px] h-[111px] shadow transition hover:shadow-lg hover:bg-blue-50 cursor-pointer${idx % 2 === 0 ? " mt-2" : ""}`}
						style={{ boxShadow: "0 2px 8px 0 rgba(0,0,0,0.04)" }}
					>
						<div className="flex items-center h-[80px] ">
							<span className="font-bold text-[12.28px]">
								{cleanText(info.title)}
							</span>
						</div>

						<div className="text-[11px] text-gray-400 ">{info.publisher}</div>
					</a>
				))}
			</div>
		</div>
